...
- **MDファイルの作成禁止**: プルリクエスト内容、提案、分析結果などは、MDファイルとして作成せず、直接エージェントの応答として提示すること
- **一時ファイルの作成を避ける**: ユーザーが確認・使用する内容は、エージェントの応答本文に直接記載すること
- **ルールファイルへの追加**: 必要に応じて、`.cursor/rules/` 配下のルールファイルに情報を追加することは許可される
