...
### 自動コミット・プッシュについて
⚠️ **重要**: セキュリティのため、実際のcommit/pushは必ず人間が確認してから実行してください。

### コミットメッセージ作成支援
変更内容を分析して、以下のフォーマットでコミットメッセージを**提案**してください：
...
### プルリクエスト作成支援
ブランチ作業完了時は、以下のテンプレートに沿ってPR文章を**提案**してください：
...
### 開発フロー
1. 変更内容の実装
2. Claude Codeにコミットメッセージ提案を依頼
3. **人間が内容確認後、手動でcommit/push**
4. PR作成時はClaude CodeにPR文章作成を依頼
5. 内容確認後、PRを作成

### 推奨するセキュリティ設定
まず `.claude/settings.json` で安全性を確保：
```json
{
  "permissions": {
    "deny": [
